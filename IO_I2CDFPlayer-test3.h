...
public:
  ...
  void _begin() override {
    ...
    //*********************************************************************************
    // Actual SC16IS752 initialization done in loop() as experiment to use non-blocking
    // ********************************************************************************
    
    I2CManager.begin();
    I2CManager.setClock(1000000);
    if (I2CManager.exists(_I2CAddress)){ // We have an I2C device on this address
  ...
  void _loop(unsigned long currentMicros) override {
    ...
    if (_deviceState_0 == DEVSTATE_INITIALISING){ // We still need to initialize the UART first
      if (_uartSent == _UART_1){ // Init UART 0
        if(_rb.isBusy()) {
          #ifdef DIAG_I2CDFplayer_playing
            DIAG(F("I2CDFPlayer: %s, UART 0 _rb.isBusy"), _I2CAddress.toString());
          #endif
          return;
        } else {
            #ifdef DIAG_I2CDFplayer_playing
              DIAG(F("I2CDFPlayer: %s, UART 0 initializing"), _I2CAddress.toString());
            #endif
            //_deviceState_0 = DEVSTATE_NORMAL; // Temp work around
            Init_SC16IS752(_UART_0); 
            _uartSent = _UART_0; // next time init UART 1     
          }
      }
    }

    if (_deviceState_1 == DEVSTATE_INITIALISING){ // We still need to initialize the UART first
      if(_uartSent == _UART_0){ // Init UART 1
        if(_rb.isBusy()) {
          #ifdef DIAG_I2CDFplayer_playing
            DIAG(F("I2CDFPlayer: %s, UART 1 _rb.isBusy"), _I2CAddress.toString());
          #endif
          return;
        } else {
            #ifdef DIAG_I2CDFplayer_playing
              DIAG(F("I2CDFPlayer: %s, UART 1 initializing"), _I2CAddress.toString());
            #endif
            _deviceState_1 = DEVSTATE_NORMAL; // Temp work around
          //Init_SC16IS752(_UART_1); // next time init UART 0
          _uartSent = _UART_1;
         }
      }  
    }
  ...
  void processIncoming(unsigned long currentMicros, uint8_t _uart) {
    ...
    if(_uart == _UART_0){ // Process uart 0
      if (FIFO_RX_LEVEL_0 >= 10) {      
        ...
        #endif
        _outbuffer_0[0] = REG_RHR << 3 | _uart << 1;
       ...
       if (FIFO_RX_LEVEL_1 >= 10) {      
        ...
        #endif
        _outbuffer_1[0] = REG_RHR << 3 | _uart << 1;
        // Only copy 10 bytes from RX FIFO, there maybe additional partial return data after a track is finished playing in the RX FIFO
    ...
    // Tranfer _inbuffer_0 or _inbuffer_1 to _inbuffer (common buffer to process incomming frame from DFPLayer)
...
private: 
  ...
  void sendPacket(uint8_t command, uint8_t arg1 = 0, uint8_t arg2 = 0, uint8_t _uart = 0) { 
   ...
   if(_uart == _UART_0){
    ...
    TX_fifo_lvl(_uart);
    if(_uart == _UART_0){
       if(FIFO_TX_LEVEL_0 > 0){ //FIFO uart 0 is empty, proceed
        
        //test
          do{
            _tmp_reg = Read_LSR_THR(_uart);
            #ifdef DIAG_I2CDFplayer_playing
              if((_tmp_reg & 0x20) == 1){
              DIAG(F("I2CDFPlayer: _tmp_reg 0x%x  _uart: %d"),_tmp_reg, _uart);
              }
            #endif         
          } while (( _tmp_reg & 0x20) == 0);
          #ifdef DIAG_I2CDFplayer_playing
              DIAG(F("I2CDFPlayer: _tmp_reg 0x%x  _uart: %d"),_tmp_reg, _uart);
          #endif
        //test
        ...
        if(FIFO_TX_LEVEL_1 > 0){ //FIFO uart 1 is empty, proceed
          //test
          do{
            _tmp_reg = Read_LSR_THR(_uart);
            #ifdef DIAG_I2CDFplayer_playing
...
uint8_t Read_LSR_THR (uint8_t _uart){
  uint8_t _rtn_val;
  UART_ReadRegister(REG_LSR, _uart);
  if(_uart == _UART_0){
    _rtn_val = _inbuffer_0[0];
  } else {
    _rtn_val = _inbuffer_1[0];
  }
  return _rtn_val;
}
