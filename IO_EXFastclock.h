  ...
  if (FAST_CLOCK_EXISTS==true) {
      ...
      byte a,b;
        ...
        if (_clocktime != _previousclocktime) {
            ...
            LCD(6,F("Clock Time: %d"), _clocktime);
            LCD(7,F("Clock Speed: %d"), _clockrate);
            RMFT2::clockEvent(_clocktime,1);
            // Now tell everyone else what the time is.
            CommandDistributor::broadcastClockTime(_clocktime, _clockrate);

            // As the maximum clock increment is 2 seconds delay a bit
            delayUntil(currentMicros + 500000);  // Wait 500ms before checking again,
